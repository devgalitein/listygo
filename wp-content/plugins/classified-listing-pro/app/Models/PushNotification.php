...
class PushNotification {
	 ...
	 * @return void
	 */
	public function setPushTokens( array $pushTokens ) {
	...
	private function notify_user_listing_approved( $data ) {
		...
		if ( isset( $data['object'] ) && $listing = $data['object'] ) {
			...
			$message = ( new ExpoRichMessage() )
				...
				->playSound();
			$imageUrl = $listing->get_the_thumbnail_url();
			if ( $imageUrl ) {
				$message->setRichContent( [ 'image' => $imageUrl ] )
	...
	private function notify_user_chat( $data ) {
		...
		if ( !empty( $object->con_id ) && !empty( $con_object->listing_id ) ) {
			...
			if ( !empty( $object->attachments ) && is_array( $object->attachments ) ) {
				foreach ( $object->attachments as $attachment ) {
					if ( !empty( $attachment['type'] ) && $attachment['type'] === 'image' && !empty($attachment['mimeType']) && strpos($attachment['mimeType'], 'image/') === 0 && !empty( $attachment['url'] ) ) {
						$message->setRichContent( [ 'image' => $attachment['url'] ] )
							->setMutableContent( true );
						break;
					}
				}
	 ...
	 * @throws ExpoMessageException
	 */
	public function notify_news_letter( array $data ) {
		if ( !empty( $data['title'] ) && !empty( $data['body'] ) ) {
			$title = esc_html( $data['title'] );
			$body = esc_html( $data['body'] );
			$message = ( new ExpoMessage() )
				->setTitle( $title )
				->setBody( $body )
				->setData( [
	 ...
	 * @throws ExpoMessageException
	 */
	public function notify_test( array $data ) {
		if ( !empty( $data['title'] ) && !empty( $data['body'] ) ) {
			$title = esc_html( $data['title'] );
			$body = esc_html( $data['body'] );
			$listing = rtcl()->factory->get_listing( 152 );
			$imageUrl = $listing->get_the_thumbnail_url();
			$message = ( new ExpoRichMessage() )
				->setTitle( PNHelper::listingApprovedTitle( $listing ) )
				->setBody( PNHelper::listingApprovedBody( $listing ) )
	...
	public function setup_locale() {
		if ( apply_filters( 'rtcl_pn_setup_locale', true ) ) {
	...
	public function restore_locale() {
		if ( apply_filters( 'rtcl_pn_restore_locale', true ) ) {
			global $sitepress, $rtcl_wpml_rest_to_language;
			if ( $sitepress && method_exists( $sitepress, 'switch_lang' ) ) {
