...
class Membership {
	...
	public function is_valid_for_free( int $cat_id ): bool {
		...
		$this->set_settings();
		$cats = isset( $this->settings['categories_of_free_ads'] ) && is_array( $this->settings['categories_of_free_ads'] )
			? $this->settings['categories_of_free_ads'] : [];
		if ( empty( $cats ) ) {
			return true;
		}
	...
	public function hasUnlimitedFreeAds(): bool {
		...
		return isset( $this->settings['enable_free_ads'] ) && $this->settings['enable_free_ads'] == "yes"
		       && isset( $this->settings['unlimited_free_ads_membership'] )
		       && 'yes' === $this->settings['unlimited_free_ads_membership'];
	}
	...
	public function apply_promotion( array $data, WP_Error $errors ): array {
		...
		if ( empty( $raw_promotions ) ) {
			$errors->add( 'rtcl_membership_promotion_invalid_promotion',
				esc_html__( "Please select a membership promotion. membership_promotions is missing", "classified-listing-store" ) );
		}
		...
		if ( $this->is_expired() || empty( $promotions = $this->get_promotions() ) ) {
			$errors->add( 'rtcl_membership_promotion_invalid_listing',
				esc_html__( "You have not membership or any promotions to promote this ad.", "classified-listing-store" ) );
		}
		$promotions_data = [];
		if ( ! $errors->has_errors() ) {
			foreach ( $raw_promotions as $raw_promotion ) {
				if ( empty( $promotions[ $raw_promotion ]['ads'] ) || empty( absint( $promotions[ $raw_promotion ]['validate'] ) )
				     || empty( $ads
						= absint( $promotions[ $raw_promotion ]['ads'] ) )
				     || empty( $validate = absint( $promotions[ $raw_promotion ]['validate'] ) )
				) {
					$errors->add( 'rtcl_membership_promotion_no_' . $raw_promotion . '_promotion',
						sprintf( __( "You have no %s promotion left.", "classified-listing-store" ),
							RtclOptions::get_listing_promotions()[ $raw_promotion ] ) );
				} else {
	...
	public function update_membership( Payment $payment ) {
		if ( $this->id ) {
			...
			$current_date = new \DateTime( current_time( 'mysql' ) );
			$expiry_date  = new \DateTime( Functions::datetime( 'mysql', trim( $this->Subscription->expiry_date ) ) );
			if ( apply_filters( 'rtcl_store_membership_carry_forward', true )
			     && ( $current_date < $expiry_date
			          || apply_filters( 'rtcl_store_expired_membership_carry_forward', false ) )
			) {
				if ( $current_date > $expiry_date ) {
					...
					if ( ! empty( $mPromotions ) ) {
						...
						foreach ( $big_promotions as $promotion_key => $promotion ) {
							if ( ! empty( $mPromotions[ $promotion_key ]['ads'] ) && $old_ads = absint( $mPromotions[ $promotion_key ]['ads'] ) ) {
								$new_ads      = ( ! empty( $promotions[ $promotion_key ]['ads'] ) ? absint( $promotions[ $promotion_key ]['ads'] ) : 0 )
								                + $old_ads;
								$new_validate = ! empty( $promotions[ $promotion_key ]['validate'] ) ? absint( $promotions[ $promotion_key ]['validate'] ) : 0;
								$new_validate = ! $new_validate && ! empty( $mPromotions[ $promotion_key ]['validate'] )
									? absint( $mPromotions[ $promotion_key ]['validate'] ) : $new_validate;
	...
	public function update_meta( $meta_key, $meta_value, $prev_value = '' ) {
		...
		global $wpdb;
		$meta_ids = $wpdb->get_col( $wpdb->prepare( "SELECT meta_id FROM {$this->table_meta} WHERE meta_key = %s AND membership_id = %d", $meta_key,
			$this->id ) );
		if ( empty( $meta_ids ) ) {
