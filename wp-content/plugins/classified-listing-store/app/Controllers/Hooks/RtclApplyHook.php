...
class RtclApplyHook {
    ...
    static function init() {
        add_filter( 'rtcl_register_settings_group', [ __CLASS__, 'add_membership_tab_item_at_settings_tabs_list' ] ); // Admin Hook
        ...
        add_filter( 'rtcl_misc_media_settings_options', [ __CLASS__, 'add_misc_options' ] );
        ...
        remove_action( 'rtcl_payment_receipt', [ TemplateHooks::class, 'payment_receipt_pricing_info' ], 20 );
        ...
        add_filter( 'rtcl_ajax_filter_load_data_query_args', [ __CLASS__, 'add_store_listing_compatibility' ] );
    ...
    public static function add_store_listing_compatibility( $args ) {
        if ( !empty( $_REQUEST['rtcl_store_id'] ) ) {
            $store_id = absint( $_REQUEST['rtcl_store_id'] );
            $args['author'] = get_post_meta( $store_id, 'store_owner_id', true );
        }
    ...
    public static function add_store_manager_listing_for() {
        if ( isset( $_GET['manager'] ) && ( $current_user_store = StoreFunctions::get_current_user_store() )
            && ( $manager = get_user_by( 'login', sanitize_text_field( $_GET['manager'] ) ) )
            && $current_user_store->owner_id() !== $manager->ID
            && in_array( $manager->ID, $current_user_store->get_manager_ids(), true )
        ) {
            ...
            <div class="rtcl-account-sub-menu">
                <ul>
                    <li class="rtcl-my-listing<?php echo !isset( $_GET['store_listing'] ) ? ' active' : ''; ?>">
                        <a href="<?php echo esc_url( Link::get_account_endpoint_url( 'listings' ) ) ?>"><?php esc_html_e( "My Listing",
                                "classified-listing-store" ); ?></a>
                    </li>
                    <li class="rtcl-store-listing<?php echo isset( $_GET['store_listing'] ) ? ' active' : ''; ?>">
                        <a href="<?php echo esc_url( add_query_arg( 'store_listing', 1,
                            Link::get_account_endpoint_url( 'listings' ) ) ) ?>"><?php esc_html_e( "Listing for store", "classified-listing-store" ); ?></a>
                    </li>
                </ul>
            </div>
            <?php
    ...
    public static function add_store_manager_listing_args( $args ) {
        ...
        if ( isset( $_GET['manager'] ) && $current_user_store && ( $manager = get_user_by( 'login', sanitize_text_field( $_GET['manager'] ) ) )
            && $current_user_store->owner_id() !== $manager->ID
            && in_array( $manager->ID, $current_user_store->get_manager_ids(), true )
        ) {
            $meta_query = [
     ...
     * @param array $request
     */
    public static function add_manager_id_after_save_new_listing( $listing, $type, $category, $new_listing_status, $request ) {
        if ( empty( $request['data']['isAdminEnd'] ) && $type === "new" && is_a( $listing, Listing::class )
            && $listing->get_owner_id() !== get_current_user_id()
        ) {
    ...
    public static function update_store_id_as_owner_for_manager_post( $post_arg, $type ) {
        ...
        if ( $type === 'new' && $current_user_id && isset( $post_arg['post_author'] )
            && absint( $post_arg['post_author'] )
            && ( $store = StoreFunctions::get_manager_store( $current_user_id ) )
    ...
    public static function update_gallery_temp_post_id_as_owner_for_manager_post( $post_arg ) {
        $current_user_id = get_current_user_id();
        if ( $current_user_id && isset( $post_arg['post_author'] ) && absint( $post_arg['post_author'] )
            && ( $store
                = StoreFunctions::get_manager_store( $current_user_id ) )
            && $post_arg['post_author'] !== $store->owner_id()
        ) {
            $post_arg['post_author'] = $store->owner_id();
        }
    ...
    public static function manager_can_manage( $user_can, $capability, $listing ) {
        ...
        $manager_id = absint( get_post_meta( $listing->get_id(), '_rtcl_manager_id', true ) );
        if ( $manager_id && $manager_id === get_current_user_id()
            && in_array( $capability, [
                'edit_rtcl_listing',
                'delete_rtcl_listing'
            ] )
        ...
        if ( isset( $_POST['_rtcl_membership_promotions'] ) ) {
            ...
            if ( !empty( $promotions_list ) ) {
                foreach ( $promotions_list as $promotion_key => $promotion_label ) {
                    if ( !empty( $_POST['_rtcl_membership_promotions'][$promotion_key]['ads'] )
                        && !empty( $_POST['_rtcl_membership_promotions'][$promotion_key]['validate'] )
                    ) {
    ...
    public static function add_pricing_options( $data, $post ) {
            ...
            foreach ( $promotions_list as $promotion_key => $promotion_label ) {
                $promotion_options .= sprintf( '<div class="rtcl-promotion-item">
                                                    ...
                                                    <div class="rtcl-promotion-action" id="%2$s">
                                                        <input name="_rtcl_membership_promotions[%1$s][ads]" class="rtcl-form-control" type="number" step="1" value="%4$d">
                                                        <input name="_rtcl_membership_promotions[%1$s][validate]" class="rtcl-form-control" type="number" step="1" value="%5$d">
                                                    </div>
                                                </div>',
                    $promotion_key,
        ...
        $data['allowed'] = substr_replace( $data['allowed'], " allowed", 20, 0 );
        $data['allowed'] = substr_replace( $data['allowed'], $hide_allowed, 4, 0 );

        $pricing_type_data = [
            'price_type' => sprintf( '<div class="rtcl-row rtcl-form-group">
                                                    <label class="rtcl-col-2 rtcl-field-label"
                                                           for="pricing-type">%s</label>
                                                    <div class="rtcl-col-10">
                                                        <select class="rtcl-form-control" id="pricing-type" name="pricing_type" required>%s</select>
                                                    </div>
                                                </div>',
        ...
        $membership_options_data = [
            'regular_ads' => sprintf( '<div class="rtcl-row rtcl-form-group regular-listings"%s>
                                            <label class="rtcl-col-2 rtcl-field-label" for="regular-listings">%s</label>
                                            <div class="rtcl-col-10">
                                                <input type="number" step="1" name="regular_ads" id="regular-listings" value="%d"
                                                       class="rtcl-form-control" required>
                                                <div class="rtcl-hints">%s</div>
                                            </div>
                                        </div>',
                $pricing_type == "membership" ? '' : " style='display:none'",
                esc_html__( "Total Number of ads", "classified-listing-store" ),
                esc_attr( $regular_ads ),
                esc_html__( "Total Number of ads for Pay per ad", "classified-listing-store" )
            ),
            'promotions'  => sprintf( '<div class="rtcl-row rtcl-form-group rtcl-membership-promotions"%s>
                                            <label class="rtcl-col-2 rtcl-field-label">%s<small class="hint d-block text-danger">(%s)</small></label>
                                            <div class="rtcl-col-10 rtcl-promotions-wrap">%s</div>
                                        </div>',
                $pricing_type == "membership" ? '' : " style='display:none'",
        ...
        ob_start();
        ?>
        <div class="rtcl-row rtcl-form-group membership-categories"<?php if ( $pricing_type != "membership" ) { ?> style="display:none"<?php } ?>>
            <label class="rtcl-col-2 rtcl-field-label"><?php esc_html_e( "Categories", "classified-listing-store" ); ?></label>
            <div class="rtcl-col-10">
                <div class="checkbox">
                    <?php
                ...
                <span class="description"><?php esc_html_e( "If you leave it unchecked, membership will validate for all categories.",
                        "classified-listing-store" ); ?></span>
            </div>
        </div>
        <?php
    ...
    public static function add_membership_tab_options( $fields, $active_tab ) {
        if ( 'membership' == $active_tab ) {
            $fields = [
                'membership_section'                  => [
                    'title'       => esc_html__( 'Membership Settings', 'classified-listing-store' ),
                    'type'        => 'section',
                    'description' => '',
                ],
                'enable'                              => [
                    'title'       => esc_html__( 'Enable Membership', 'classified-listing-store' ),
                    'label'       => esc_html__( 'Enable', 'classified-listing-store' ),
                    'type'        => 'checkbox',
                    'description' => esc_html__( 'Enable Membership option', 'classified-listing-store' ),
                ],
                'enable_store'                        => [
                    'title'       => esc_html__( 'Enable Store', 'classified-listing-store' ),
                    'label'       => esc_html__( 'Enable', 'classified-listing-store' ),
                    'type'        => 'checkbox',
                ...
                'enable_store_rating'                 => [
                    'title'       => esc_html__( 'Enable Store Rating', 'classified-listing-store' ),
                    'label'       => esc_html__( 'Enable', 'classified-listing-store' ),
                    'type'        => 'checkbox',
                    'default'     => 'yes',
                ...
                'new_store_status'                    => [
                    'title'       => esc_html__( 'New / Edited Store Status', 'classified-listing-store' ),
                    'type'        => 'select',
                    'default'     => 'publish',
                    'options'     => [
                        'publish' => esc_html__( 'Publish', 'classified-listing-store' ),
                        'pending' => esc_html__( 'Pending', 'classified-listing-store' ),
                    ],
                    'description' => esc_html__( 'Store default status after creation.', 'classified-listing-store' )
                ],
                'enable_store_only_membership'        => [
                    'title'       => esc_html__( 'Store Only for Membership', 'classified-listing-store' ),
                    'label'       => esc_html__( 'Enable', 'classified-listing-store' ),
                    'type'        => 'checkbox',
                    'description' => esc_html__( 'Store menu at My Account page will visible only for the valid membership users. ',
                        'classified-listing-store' ),
                    'dependency'  => [
                        'rules' => [
                            '#rtcl_membership_settings-enable_store' => [
                ...
                'display_store_only_valid_membership' => [
                    ...
                    'label'       => esc_html__( 'Enable', 'classified-listing-store' ),
                    'type'        => 'checkbox',
                    'description' => esc_html__( 'Single store page will display only for valid membership owner. If enable, store single page will be only visible until membership is active.',
                        'classified-listing-store' ),
                    'dependency'  => [
                        'rules' => [
                            '#rtcl_membership_settings-enable_store' => [
                ...
                'enable_pay_per_ad'                   => [
                    'title'       => esc_html__( 'Enable Pay Per Ad', 'classified-listing-store' ),
                    'label'       => esc_html__( 'Enable', 'classified-listing-store' ),
                    'type'        => 'checkbox',
                    'description' => esc_html__( 'Enable Pay Per Ad posting', 'classified-listing-store' ),
                    'dependency'  => [
                        'rules' => [
                            '#rtcl_membership_settings-enable' => [
                                'type'  => 'equal',
                ...
                'enable_free_ads'                     => [
                    'title'       => esc_html__( 'Enable Free Ads', 'classified-listing-store' ),
                    'label'       => esc_html__( 'Enable', 'classified-listing-store' ),
                    'type'        => 'checkbox',
                    ...
                    'dependency'  => [
                        'rules' => [
                            ...
                            '#rtcl_membership_settings-enable_pay_per_ad' => [
                                'type'  => 'notequal',
                                'value' => 'yes'
                            ],
                        ]
                ...
                'number_of_free_ads'                  => [
                    ...
                    'type'        => 'number',
                    'default'     => 3,
                    'description' => __( 'Number of ads to post as free with out membership, if membership is enabled.<br>If this field is blank dy default it will be 3',
                        'classified-listing-store' ),
                    'dependency'  => [
                        'rules' => [
                            '#rtcl_membership_settings-enable'            => [
                ...
                'unlimited_free_ads_membership'       => [
                    'title'       => esc_html__( 'Enable Unlimited Free Ads for Membership', 'classified-listing-store' ),
                    'label'       => esc_html__( 'Enable', 'classified-listing-store' ),
                    'type'        => 'checkbox',
                    'dependency'  => [
                        'rules' => [
                ...
                'categories_of_free_ads'              => [
                    ...
                    'description' => esc_html__( 'Select the specific category for free ads, Leave it un select to allow any category.',
                        'classified-listing-store' ),
                    'dependency'  => [
                        'rules' => [
                            '#rtcl_membership_settings-enable'          => [
        ...
        if ( $position > -1 ) {
            $option = [
                ...
                'store_logo_size'   => [
                    ...
                    'description' => esc_html__( 'This image size is being used at the store detail page and where store link is given.',
                        "classified-listing-store" )
                ]
            ];
    ...
    public static function get_template_part( $template, $slug, $name ) {
        if ( strpos( $template, "classified-listing/" . $name ) === false && !file_exists( $template ) ) {
            ...
            if ( !$template ) {
                if ( $name ) {
                    $template = RTCL_STORE_TEMPLATE_DEBUG_MODE
                        ? ''
                        : locate_template(
                            [
