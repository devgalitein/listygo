...
namespace RtclBooking\Models;

use Rtcl;

class Dependencies {
	const MIN_RTCL = '5.0.0';
	...
	public function _missing_plugins_warning() {
		...
		foreach ( $this->missing as $title => $url ) {
			...
			if ( $counter == sizeof( $this->missing ) ) {
				...
				$sep = ' ' . esc_html__( 'and', 'rtcl-booking' ) . ' ';
			} else {
				$sep = ', ';
			}
        ...
        <div class="notice notice-error">
            <p>
				<?php echo wp_kses( sprintf( __( '<strong>Classified Listing - Bookings</strong> is enabled but not effective. It requires %s in order to work.', 'rtcl-booking' ), $missing ),
					[
						'strong' => [],
						'a'      => [
							'href'  => true,
	...
	public function _old_rtcl_warning() {
		...
		$message = wp_kses( __( sprintf( '<strong>Classified Listing - Bookings</strong> is enabled but not effective. It is not compatible with <a class="thickbox open-plugin-details-modal" href="%1$s">Classified Listing</a> versions prior %2$s.',
			$link,
			self::MIN_RTCL
		), 'rtcl-booking' ), [ 'strong' => [], 'a' => [ 'href' => true, 'class' => true ] ] );
