...
echo '<div class="rtcl-listing-side-title"><h3>'.esc_html__( 'Open Hours', 'listygo' ).'</h3></div>';
...
?>
    ...
    <table class="rtclbh">
        <?php if ($options['footer']) : ?>
            ...
            <tr>
                <th>&nbsp;</th>
                <th><?php esc_html_e(!empty($options['open_text']) ? $options['open_text'] : esc_html__('Open', 'listygo')); ?></th>
                <th class="rtclbh-separator">&nbsp;</th>
                <th><?php esc_html_e(!empty($options['close_text']) ? $options['close_text'] : esc_html__('Close', 'listygo')); ?></th>
            </tr>
            </tfoot>
        <?php endif; ?>
        <tbody>
        ...
        foreach (Options::get_week_days() as $dayKey => $day) {
            ...
            if (!BHS::openToday($dayData)) {
                if ($options['show_closed_day']) {
                    printf('<tr class="rtclbh-closed %1$s"><th>%2$s</th><td class="rtclbh-info" colspan="3">%3$s</td></tr>',
                        ...
                        esc_attr($day),
                        $current_week_day === $dayKey ? (!empty($options['closed_today_text']) ? $options['closed_today_text'] : esc_html__('Closed Today', 'listygo'))
                            : (!empty($options['closed_24_text']) ? $options['closed_24_text'] : esc_html__('Closed', 'listygo'))
                    );
                }
            ...
            if (BHS::isOpenAllDayLong($dayData)) {
                printf('<tr class="rtclbh-opened %1$s"><th>%2$s</th><td class="rtclbh-info" colspan="3">%3$s</td></tr>',
                    sprintf('rtclbh-day-%d%s', absint($dayKey), $current_week_day === $dayKey ? ' rtclbh-active' : ''),
                    esc_attr($day),
                    $current_week_day === $dayKey ? (!empty($options['open_today_text']) ? $options['open_today_text'] : esc_html__('Open Today (24 Hours)', 'listygo'))
                        : (!empty($options['open_24_text']) ? $options['open_24_text'] : esc_html__('Open (24 Hours)', 'listygo'))
                );
...
<?php
    ...
    if ($options['show_open_status']) {
        if (BHS::openStatus($business_hours)) {
            printf('<div class="rtclbh-status rtclbh-status-open">%s</div>', !empty($options['open_status_text']) ? $options['open_status_text'] : esc_html__('We are currently open.', 'listygo'));
        } else {
            printf('<div class="rtclbh-status rtclbh-status-closed">%s</div>', !empty($options['close_status_text']) ? $options['close_status_text'] : esc_html__('Sorry, we are currently closed.', 'listygo'));
        }
    }
echo '</div>';

//    error_log( print_r( $business_hours, true ) . "\n\n" , 3, __DIR__ . '/log.txt' );
